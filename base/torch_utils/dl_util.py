...
def get_optimizer(model,
    ...
    img_lr = float(kwargs.get('img_lr', lr))
    for name, param in model.named_parameters():
        freeze_flag = False
    ...
    if params is None:
        if weight_decay:
            ...
            param_configs = get_parameter_groups(model, img_lr, weight_decay, skip, assigner)
            weight_decay = 0.
        else:
...
def get_parameter_groups(model, img_lr, weight_decay, skip_list=(), assigner=None):
    ...
    for name, param in model.named_parameters():
        ...
        if 'image_encoder' in name:
            if len(param.shape) == 1 or name.endswith(".bias") or name.split('.')[-1] in skip_list:
                group_name = "img_encoder_no_decay"
                this_weight_decay = 0.
            else:
                group_name = "img_encoder_decay"
